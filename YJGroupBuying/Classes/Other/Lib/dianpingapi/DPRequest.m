    ...
    if (!result) { // 服务器的东西是空，错误
		...
		if ([status isEqualToString:@"OK"]) {
			...
			if ([status isEqualToString:@"ERROR"]) {
                ...
                NSString *msg = errorDict[@"errorMessage"];
                NSString *code = errorDict[@"errorCode"];
                NSError *error = [NSError errorWithDomain:msg code:[code intValue] userInfo:nil];
                [self failedWithError:error];
